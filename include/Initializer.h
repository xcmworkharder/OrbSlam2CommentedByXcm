...
/**
 * 模块说明:用于单目计算,不适用于双目和RGBD情况
 *      初始化的方法是根据当前帧和参考帧匹配得到的特征点对，利用RANSAC方法去计算单应性矩阵H和基础矩阵F
 *      然后根据重投影误差大小决定选择哪个矩阵，最后使用SFM方法利用矩阵计算旋转R和平移T。
 *      之所以要同时计算单应性矩阵H和基础矩阵F，是因为当特征点在同一个平面上时，基础矩阵F会出现退化现象
