...
void LocalMapping::Run() {
    ...
    /// 主循环
    while (1) {
        /// 通知Tracking线程,localmapping is busy
        SetAcceptKeyFrames(false);

        /// 检查关键帧列表是否不为空
        if (CheckNewKeyFrames()) { /// 检查是否有等待插入的关键帧
            /// 处理列表中的关键帧
            ProcessNewKeyFrame();
...
void LocalMapping::ProcessNewKeyFrame() {
    ...
    /// 计算该关键帧特征点的Bow映射关系,得到该关键帧对应的词袋向量
    ...
    for (size_t i = 0; i < vpMapPointMatches.size(); i++) {
        ...
        if (pMP) {
            if (!pMP->isBad()) {
                ...
                } else { /// 只会发生在跟踪中插入新的双目点情况，当前帧中有地图点，但地图点中更没有关键帧信息
                    /// 当前帧生成的MapPoints
 ...
 * 删除地图点
 *     1）如果地图点被标记为bad
 *     2) IncreaseFound / IncreaseVisible < 25%, 跟踪与以及可观测的比例不足25%
 *     3) 未被超过2个关键帧看淡，并且当前关键帧id和看到该点的第一个关键帧之间id差大于等于2时
 *     4) 当前关键帧id和看到该点的第一个关键帧之间id之差大于等于3时
 */
void LocalMapping::MapPointCulling() {
    ...
    while (lit != mlpRecentAddedMapPoints.end()) {
        ...
        if (pMP->isBad()) {
            ...
            /// 从该点建立开始到现在已经过了不小于2个关键帧,但是观测到该点的关键帧数却不超过cnThObs帧，那么该点检验不合格
            pMP->SetBadFlag();
            lit = mlpRecentAddedMapPoints.erase(lit);
        } else if (((int)nCurrentKFid - (int)pMP->mnFirstKFid) >= 3)
            /// 从建立该点开始已经过了3个关键帧而没有被剔除，则认为是质量高的点
...
void LocalMapping::CreateNewMapPoints() {
    ...
    /// 用于后面的点深度的验证, 1.5可能为经验值
    const float ratioFactor = 1.5f * mpCurrentKeyFrame->mfScaleFactor;

    /// 初始化三角化成功的地图点的计数
    int nnew = 0;

    /// 遍历相邻关键帧vpNeighKFs查找满足对极约束，进行三角化
    for (size_t i = 0; i < vpNeighKFs.size(); i++) {
        ...
        /// 匹配pKF1与pKF2之间未被匹配的特征点并通过bow加速，并校验是否符合对级约束
...
void LocalMapping::SearchInNeighbors() {
    ...
    for (vector<KeyFrame*>::const_iterator vit = vpNeighKFs.begin(),
        ...
        for (vector<KeyFrame*>::const_iterator vit2 = vpSecondNeighKFs.begin(),
            ...
            KeyFrame* pKFi2 = *vit2;
            /// 这个二级关键帧要求没有和当前关键帧发生融合,并且这个二级关键帧也不是当前关键帧
    ...
    /// 将当前帧的MapPoints分别与一级二级相邻帧(的MapPoints)进行融合 -- 正向
    ...
    /// 将一级二级相邻帧的MapPoints分别与当前帧（的MapPoints）进行融合 -- 反向
    /// 遍历每一个一级邻接和二级邻接关键帧
    ...
    /// 更新共视图中当前帧的MapPoints后更新与其它帧的连接关系
    mpCurrentKeyFrame->UpdateConnections();
}
...
/// 关键帧剔除,在Covisibility Graph中的关键帧，其90%以上的MapPoints能被其他关键帧（至少3个）观测到
/// 则认为该关键帧为冗余关键帧
void LocalMapping::KeyFrameCulling() {
    /// 根据Covisibility Graph提取当前帧的共视关键帧 (所有)
