...
/**
 * 全局地图优化：最大范围的优化,包含所有相机位姿和地图点XYZ位置
 *     使用场景：
...
void Optimizer::BundleAdjustment(const vector<KeyFrame*>& vpKFs, const vector<MapPoint*>& vpMP,
    ...
    /// 得到优化的结果,遍历所有的关键帧(因为只要是关键帧没有不是bad的话,就一定会参与到GBA的过程中)
    for (size_t i = 0; i < vpKFs.size(); i++) {
        KeyFrame* pKF = vpKFs[i];
        if (pKF->isBad())
            continue;
...
/**
 * @brief 当前帧优化, 仅优化当前帧位姿,不优化地图点
 *     用于Tracking线程中：运动跟踪、参考帧跟踪、地图跟踪、重定位
 *     1. Vertex: g2o::VertexSE3Expmap()，即当前帧的Tcw
 *     2. Edge:
...
int Optimizer::PoseOptimization(Frame* pFrame) {
    /// 构造g2o优化器, BlockSolver_6_3表示：位姿_PoseDim为6维,路标点_LandmarkDim是3维
    g2o::SparseOptimizer optimizer;
    /// 创建线性求解器
    g2o::BlockSolver_6_3::LinearSolverType* linearSolver =
            new g2o::LinearSolverDense<g2o::BlockSolver_6_3::PoseMatrixType>();
    /// 创建BlocSolver, 用线性求解器初始化
    g2o::BlockSolver_6_3* solver_ptr = new g2o::BlockSolver_6_3(linearSolver);
    /// 使用优化算法,这里使用LM
    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
    optimizer.setAlgorithm(solver);
    /// 输入帧中,有效的参与优化过程的2D-3D点对
    ...
    /// 添加一元边
    {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (pMP) {
                ...
                }  else { /// 双目的观测，包括RGBD虚拟出的双目，与单目的区别是观测多了一个kp_ur，由二维变成了三维
                    nInitialCorrespondences++;
                    /// 特征点初始化为内点
                    pFrame->mvbOutlier[i] = false;
    ...
    for (size_t it = 0; it < 4; it++) {
        vSE3->setEstimate(Converter::toSE3Quat(pFrame->mTcw));
        /// 只对level为0的边优化,0代表inlier, 1代表outlier
        ...
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
            ...
            /// 误差边来自于outlier,不参与计算,重新计算误差(g2o只会计算active edge的误差)
            ...
            if (chi2 > chi2Mono[it]) {
                ...
                e->setLevel(1);                 // 设置为outlier,level为1
                nBad++;
            } else {
                pFrame->mvbOutlier[idx] = false;
                e->setLevel(0);                 // 设置为inlier,level为0
            }
            /// 除了前两次优化需要RobustKernel以外, 其余都不需要,因为重投影的误差已经有明显的下降了
            if (it == 2)
                e->setRobustKernel(0);
        }
        /// 同样的原理遍历双目的误差边
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag, Map* pMap) {
    ...
    /// 将当前关键帧及其共视关键帧加入lLocalKeyFrames
    ...
    /// 添加不优化的顶点：Pose of Fixed KeyFrame，注意这里调用了vSE3->setFixed(true)
    ...
    for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(),
        ...
        vPoint->setId(id);
        /// 使用舒尔补,先消去地图点可以先求出位姿,再求出路标点,提高效率
        vPoint->setMarginalized(true);  
        optimizer.addVertex(vPoint);
        ...
        /// 在添加完了一个地图点之后, 对每一对关联的MapPoint和KeyFrame构建边
        for (map<KeyFrame*, size_t>::const_iterator mit = observations.begin(),
                     mend = observations.end(); mit != mend; mit++) {
            KeyFrame* pKFi = mit->first;
            if (!pKFi->isBad()) {
    ...
    /// 如果有外部请求停止,那么就不再进行第二阶段的优化
    if (bDoMore) {
        /// 遍历所有的单目误差边,找到outlier,设置level为1,下次不优化
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend;i++) {
            ...
            /// 自由度为2的卡方分布，显著性水平为0.05，对应的临界阈值5.991
            /// 如果当前边误差超出阈值，或者边链接的地图点深度值为负，说明这个边有问题，不优化了。
    ...
    for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
        ...
        /// 基于卡方检验计算出的阈值（假设测量有一个像素的偏差）
        /// 自由度为2的卡方分布，显著性水平为0.05，对应的临界阈值5.991
        /// 如果当前边误差超出阈值，或者边链接的地图点深度值为负，说明这个边有问题，要删掉了
        if (e->chi2() > 5.991 || !e->isDepthPositive()) {
            /// outlier
...
/**
 * @brief 全局地图调整（简化版），闭环检测后，EssentialGraph优化
 *      1. Vertex:
...
void Optimizer::OptimizeEssentialGraph(Map* pMap, KeyFrame* pLoopKF, KeyFrame* pCurKF,
    ...
    for(size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
        ...
        /// 添加在CorrectLoop函数中AddLoopEdge函数添加的闭环连接边（当前帧与闭环匹配帧之间的连接关系）(这里面也包括了当前遍历到的这个关键帧之前曾经存在过的回环边)
    ...
    /// 步骤5和步骤6优化, 得到关键帧的位姿后，MapPoints根据参考帧优化前后的相对关系调整自己的位置
    /// 遍历所有地图点
    for(size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
        ...
        int nIDr;
        /// 该MapPoint经过Sim3调整过，(LoopClosing.cpp，CorrectLoop函数
...
int Optimizer::OptimizeSim3(KeyFrame* pKF1, KeyFrame* pKF2,
    ...
    /// 先构造求解器
    g2o::SparseOptimizer optimizer;
    /// 构造线性方程求解器, BlockSolverX表示误差变量和误差项维度是动态的
    ...
    /// 获取当前关键帧，和特征点相匹配的地图点,这里用pKF1的所有地图点
    ...
    /// 观测是二维的，对应的学生t分布的自由度为2, 当自由度趋于无穷时,t分布等同于标准正态分布
    ...
    for (int i = 0; i < N; i++) {
        ...
        nCorrespondences++;
        /// 添加误差项边 edge x1 = S12 * X2
        Eigen::Matrix<double,2,1> obs1;
        const cv::KeyPoint &kpUn1 = pKF1->mvKeysUn[i];
        obs1 << kpUn1.pt.x, kpUn1.pt.y;
        /// 添加两个顶点（3D点）到相机投影的边 -- 投影到当前关键帧 -- 正向投影
